...
function editS(id) {
    ...
    document.getElementById('start').value = (s.type === 'work' && s.start !== "FÉRIÉ") ? s.start : "";
    document.getElementById('end').value = (s.type === 'work' && s.end !== "--:--") ? s.end : "";
...
function render() {
    ...
    data.forEach(s => {
        ...
        const icon = typeIcons[s.type] || 'bi-calendar';
        
        hist.innerHTML += `
            <div class="card p-3 mb-2 border-start border-4 ${s.type === 'ferie' ? 'bg-holiday' : 'border-primary'}">
                <div class="d-flex justify-content-between align-items-center">
                    <div onclick="editS(${s.id})" style="cursor:pointer; flex-grow:1;">
                        <small class="d-block text-uppercase text-muted">${d.toLocaleDateString('fr-FR',{weekday:'short'})} ${d.getDate()}</small>
                        <div class="fw-bold"><i class="bi ${icon} me-1"></i> ${displayTitle}</div>
                        <small class="text-muted">${s.start} - ${s.end} ${s.pause > 0 ? '(Pause '+s.pause+'m)' : ''}</small>
                    </div>
                    <div class="text-end">
    ...
    document.getElementById('details-paye').innerText = `${fH(tM)} travaillées • ${sM} paniers repas`;
}
...
function resetAll() { 
    if(confirm("ATTENTION : Cela va effacer TOUTES vos heures. Continuer ?")) { 
        localStorage.clear(); 
        data = []; 
        render(); 
    } 
...
function exportToPDF(mode) {
    ...
    document.getElementById('pdf-subtitle').innerText = "Récapitulatif mensuel";
    const opt = { margin: 10, filename: 'suivi_heures.pdf', html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
    html2pdf().set(opt).from(document.getElementById('printable-area')).save().then(() => { headerTitle.style.display = 'none'; });
}
